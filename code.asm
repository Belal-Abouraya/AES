...
;uses the s-table to change the given byte   
subbyte macro dt
    mov ax, 0000H
    mov al, dt
    mov di, ax
...
;substitutes an entire row of state matrix
subrows macro
    push cx
    mov cx, 0010h
    L1: mov si,cx 
...
;rotates all the rows of the state matrix
shufflerows macro
    push cx
    mov cx, 0003h
    mov di, 0000H
...
;does the mod multiplication by 2
prod2 macro t
    mov al, t
    mov ah, al
    and ah, 80h
...
;does the mod multiplication by 3
prod3 macro t
    mov dl, t
    prod2 dl
    xor dl, t
...
;does one element of matrix multiplication
...
;does an entire column of matrix multiplication 
...
;does the entire matrix multiplication
mixcolumns macro
    push cx
    mov cx, 0004h
    mov di, 0004h
...
;xors to memory locations directly
xor2 macro t1, t2
    mov al, t1
    xor al, t2
    mov t1, al
...
;prints the value of the given ascii code on the terminal
macro printbyte t
    mov dl, t
    mov ah, 02
    int 21h   
...
;prints the value of the number on the screen
macro printhexval t
    push bx
    mov bl, t;
    and bx, 000f0h;
...
;outputs the entire state matrix on the screen
macro output
    local L8
    local L9
    push cx
...
;takes the ascii code of one character from the terminal
inchar macro
    mov ah, 1
    int 21h
endm

;gets the numerical value of the given ascii code
getval macro t
    local let
    cmp t, 61h
    jb let
...
;takes the entire state array from the user
input macro
    local L10
    mov cx, 16
    mov bx, 0
